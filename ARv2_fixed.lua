...
MainTab:Button({
    ...
    Callback = function()
        ...
        notify("[DYHUB] All Morphs, Classes and Auras unlocked!")
    end,
})
...
                task.wait(0.1)
            end
            notify("[DYHUB] Completed Infinite Cash")
        end)
    end,
})
...
                task.wait(0.05)
            end
            notify("[DYHUB] Completed Infinite Dupe Spin")
        end)
    end,
})
    ...
    Callback = function()
        if playerNameInput ~= "" then
            ...
            if p and p.Character and LocalPlayer.Character then
                ...
                notify("[DYHUB] Player not found or character missing")
            end
        else
            notify("[DYHUB] Please enter a player name")
        end
    end,
})
...
MiscTab:Toggle({
    ...
    Value = true,
    Callback = function(state)
        notify("[DYHUB] Bypassing Checking Loop" .. (state and "Enabled" or "Disabled"))
        notify("[DYHUB] Anti Reset " .. (state and "Enabled" or "Disabled"))
        notify("[DYHUB] Anti Admin" .. (state and "Enabled" or "Disabled"))
        notify("[DYHUB] Anti Moderator" .. (state and "Enabled" or "Disabled"))
    end,
})
...
MiscTab:Toggle({
    ...
    Callback = function(state)
        antiAfk = state
        notify("[DYHUB] Anti AFK " .. (state and "Enabled" or "Disabled"))
    end,
})
...
ConfigTab:Button({
    ...
    Callback = function()
        if configNameInput ~= "" then
            ...
            notify("[DYHUB] Enter a config name before saving.")
        end
    end,
})
...
notify("[DYHUB] Full Anime Rails Loaded!")
